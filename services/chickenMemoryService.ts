...
export class ChickenBusinessMemoryService {
  ...
  async createBusinessRelation(relation: BusinessRelation): Promise<boolean> {
    ...
    try {
      ...
      console.log(`✅ Created relation: ${relation.from} --${relation.relationType}--> ${relation.to}`);
      return true;
  ...
  async searchBusinessContext(query: string): Promise<any[]> {
    ...
    try {
      const result = await this.memoryClient.request({
        method: 'tools/call',
        params: {
          name: 'search_memory',
          arguments: { query }
  ...
  private extractPotentialEntities(noteText: string): string[] {
    ...
    // Look for known suppliers
    if (text.includes('magnolia')) entities.push('Magnolia_Supplier');
